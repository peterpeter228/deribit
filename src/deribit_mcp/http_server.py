...
async def sse_endpoint(request: Request) -> EventSourceResponse:
    """    SSE endpoint for MCP (HTTP with SSE transport).

    Per MCP spec (2024-11-05): when a client connects, the server MUST immediately send an
    `endpoint` SSE event whose data is the URI the client should POST JSON-RPC messages to.

    This implementation:
    - Creates a per-connection session_id
    - Sends `event: endpoint` with a message endpoint that includes the session_id
    - Streams server->client JSON-RPC messages as `event: message` SSE events

    Notes:
    - We deliberately do NOT emit any JSON-RPC "message" events before the client sends
      `initialize`, because some MCP clients (including Cherry Studio) will wait for the
      `endpoint` event first.
    - Some clients/proxies may issue HEAD requests; we return 200 for HEAD without allocating
      a new session.
    """

    # Starlette will route HEAD to this handler when methods=["GET"].
    # Do NOT allocate sessions for HEAD; it causes "invalid/expired session_id" races.
    ...
    # Build an absolute message endpoint URL for maximum client compatibility.
    # If running behind a reverse proxy, prefer X-Forwarded-* headers when present.
    def _forwarded_url(req: Request):
        url = req.url
        xf_proto = req.headers.get("x-forwarded-proto")
        xf_host = req.headers.get("x-forwarded-host")
        host = xf_host.split(",")[0].strip() if xf_host else req.headers.get("host")
        if xf_proto:
            url = url.replace(scheme=xf_proto.split(",")[0].strip())
        if host:
            url = url.replace(netloc=host)
        return url

    # Derive message path from the SSE path to support deployments under a prefix.
    # Examples:
    #   /sse       -> /messages
    #   /mcp/sse   -> /mcp/messages
    sse_path = request.url.path.rstrip("/")
    base_path = sse_path[:-4] if sse_path.endswith("/sse") else sse_path
    message_path = (base_path + "/messages") if base_path else "/messages"

    message_url = str(_forwarded_url(request).replace(path=message_path, query=f"session_id={session_id}"))

    async def event_generator():
        try:
            # MUST be first event for MCP SSE clients
            yield {"event": "endpoint", "data": message_url}
            logger.info(f"SSE session {session_id} endpoint advertised: {message_url} (client: {client_ip})")
            ...
            while not session._closed:
                try:
                    ...
                    try:
                        if await request.is_disconnected():
                            logger.info(f"SSE connection disconnected (client: {client_ip}) for session {session_id}")
                            break
                    except Exception:
                        # If we can't detect disconnection, assume alive.
                        pass
                ...
                if isinstance(message, dict) and "event" in message and "data" in message:
                    ...
                    yield {"event": "message", "data": _compact_json(mcp_message)}
    ...
    return EventSourceResponse(
        ...
        headers={
            ...
            # Keep this header for debug/backward-compat, but MCP clients should rely on the endpoint event.
...
async def mcp_message_endpoint(request: Request) -> JSONResponse:
    ...
    # Extract session_id (query param > headers > body)
    # MCP (HTTP+SSE) clients typically POST to the endpoint URI returned by the SSE `endpoint` event,
    # which commonly includes `?session_id=...`.
    session_id = (
        ...
        return JSONResponse(
            {
                "jsonrpc": "2.0",
                "id": request_id,
                "error": {"code": -32602, "message": "Invalid Params: Missing session_id (provide via endpoint URI query ?session_id=..., or Mcp-Session-Id/X-Session-Id header, or body)"},
            },
            status_code=400,
        )
...
routes = [
    ...
    Route("/mcp/sse", sse_endpoint, methods=["GET"]),
    Route("/messages", mcp_message_endpoint, methods=["POST"]),
    Route("/message", mcp_message_endpoint, methods=["POST"]),
    Route("/mcp/messages", mcp_message_endpoint, methods=["POST"]),
